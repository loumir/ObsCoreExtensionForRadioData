...
Select any dataset map (raster or on-the-fly) with s\_resolution\_min better than the desired spatial resolution of 1 arcmin.}
...
\todo{Nb: The above queries can be run with s\_resolution\_min and s\_fov\_max instead of s\_resolution\_max and s\_fov\_min respectively, to obtain results in which at least part of the dataset satisfy the condition.
...
\subsection{ visibility data product and targer object selection }
...
\textit{Any visibility dataset around FRB 121102  where  maximum angular scale is larger than 1 minute .}\\ \\
Show me all datasets satisfying:\\
...
\subsection{Use case : uv distribution selection}
