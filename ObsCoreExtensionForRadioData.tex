...
be added.\todo{can we make rules such that such additional columns
...
It is also possible to include a view performing this JOIN in the ivoa schema in
...
\sptablerule
\texttt{s\_maximum\_angular\_scale}&\texttt{maximum scale in dataset, shortest baseline and  frequency dependant}& &{phys.angSize;stat.max}&arcsec&interferometry\cr
\sptablerule
...
\sptablerule
\texttt{uv\_distribution\_map}&\texttt{access to map}&\texttt{Char.UVAxis.\newline  Sampling.\newline Sensitivity.Map}&stat.fourier;pos;\newline meta.ref.url&&interferometry \cr
\sptablerule
